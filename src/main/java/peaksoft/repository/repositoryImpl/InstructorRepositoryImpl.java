...
public class InstructorRepositoryImpl implements InstructorRepository {
    ...
    public List<Instructor> getAllInstructors(Long courseId) {
        return  entityManager.createQuery("select c from Instructor c where c.course.id = :id",Instructor.class).setParameter("id",courseId).getResultList();
    }
        ...
        if (course.getInstructorList()!=null){
            for (Instructor i:course.getInstructorList()){
                if (i.getId()==instructorId){
                    throw new IOException("Such instructor already exists!");
                }
            }
        }
